...
const roleColors = {
  visitor: 'text-slate-400', researcher: 'text-blue-400',
  ...
  <div class="p-6 space-y-6 animate-fade-in">
    ...
    <div class="flex gap-3 flex-wrap">
      ...
      <select v-model="roleFilter" @change="doSearch"
        class="bg-slate-900/60 border border-white/10 rounded-lg px-4 py-2.5 text-sm text-slate-300 focus:outline-none focus:ring-2 focus:ring-brand-500/50 transition-all">
    ...
    <div class="bg-slate-900/60 border border-white/8 rounded-xl overflow-hidden">
      <table class="w-full">
        <thead class="border-b border-white/8">
          <tr>
            <th class="px-6 py-3.5 text-left text-xs font-semibold text-slate-400 uppercase tracking-wider">Utilisateur</th>
            <th class="px-6 py-3.5 text-left text-xs font-semibold text-slate-400 uppercase tracking-wider">Rôle</th>
            <th class="px-6 py-3.5 text-left text-xs font-semibold text-slate-400 uppercase tracking-wider">Statut</th>
            <th class="px-6 py-3.5 text-left text-xs font-semibold text-slate-400 uppercase tracking-wider">Inscription</th>
        ...
        <tbody class="divide-y divide-white/5">
          <tr v-for="user in users.data" :key="user.id" class="hover:bg-white/3 transition-colors">
            <td class="px-6 py-4">
              <div class="font-medium text-white text-sm">{{ user.prenom }} {{ user.nom }}</div>
              ...
              <div v-if="user.role === 'researcher' && user.specialite" class="text-xs text-brand-400 mt-1">
                ...
                <span class="text-slate-500" v-if="user.axe_nom">({{ user.axe_nom }})</span>
              </div>
            ...
            <td class="px-6 py-4 text-xs text-slate-500">{{ user.created_at?.substring(0, 10) }}</td>
            <td class="px-6 py-4 text-right flex justify-end gap-2">
              ...
              <button @click="openEdit(user)" class="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-medium transition-all">
    ...
    <!-- Pagination -->
    <div v-if="users.last_page > 1" class="flex items-center justify-between text-sm text-slate-400">
      <span>{{ users.from }}–{{ users.to }} sur {{ users.total }}</span>
      <div class="flex gap-2">
        <a v-if="users.prev_page_url" :href="users.prev_page_url" class="px-3 py-1.5 rounded-lg border border-white/10 hover:border-brand-500/50 hover:text-white transition-all">← Précédent</a>
        <a v-if="users.next_page_url" :href="users.next_page_url" class="px-3 py-1.5 rounded-lg border border-white/10 hover:border-brand-500/50 hover:text-white transition-all">Suivant →</a>
      </div>
    </div>
    ...
    <div v-if="editingUser" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" @click.self="editingUser = null">
      <div class="bg-slate-900 border border-white/10 rounded-2xl p-6 w-full max-w-sm shadow-glass animate-slide-up">
        <h3 class="text-lg font-semibold text-white mb-1">Modifier l'utilisateur</h3>
        <p class="text-slate-400 text-sm mb-4">{{ editingUser.prenom }} {{ editingUser.nom }}</p>
        ...
        <div v-if="editingUser.role === 'researcher' && editingUser.specialite" class="text-xs text-brand-400 bg-brand-500/5 border border-brand-500/10 rounded-lg p-3 mb-4">
          <strong class="block text-slate-400 mb-1">🔬 Spécialité Chercheur :</strong>
        ...
        <div v-if="editingUser.role === 'doctoral_student'" class="text-xs text-purple-400 bg-purple-500/5 border border-purple-500/10 rounded-lg p-3 mb-4">
          <strong class="block text-slate-400 mb-1">🎓 Domaine d'expertise Doctorant :</strong>
          {{ editingUser.domaine_expertise || 'Non renseigné' }}
          <div v-if="editingUser.axe_nom" class="mt-2 text-slate-400">
        ...
        <div class="space-y-4">
          <div>
            <label class="block text-xs font-medium text-slate-400 mb-1.5">Rôle</label>
            <select v-model="editForm.role" class="w-full bg-slate-800 border border-white/10 rounded-lg px-4 py-2.5 text-sm text-white focus:outline-none focus:ring-2 focus:ring-brand-500/50 transition-all">
              <option v-for="(label, val) in roleLabel" :key="val" :value="val">{{ label }}</option>
            </select>
          </div>
          <div>
            <label class="block text-xs font-medium text-slate-400 mb-1.5">Axe thématique principal</label>
            <select v-model="editForm.axe_principal_id" class="w-full bg-slate-800 border border-white/10 rounded-lg px-4 py-2.5 text-sm text-white focus:outline-none focus:ring-2 focus:ring-brand-500/50 transition-all">
              <option value="">— Aucun axe (Global) —</option>
              <option v-for="axe in axes" :key="axe.id" :value="axe.id">{{ axe.nom_fr }} ({{ axe.code }})</option>
            </select>
          </div>
          <div>
            <label class="block text-xs font-medium text-slate-400 mb-1.5">Statut</label>
            <select v-model="editForm.statut" class="w-full bg-slate-800 border border-white/10 rounded-lg px-4 py-2.5 text-sm text-white focus:outline-none focus:ring-2 focus:ring-brand-500/50 transition-all">
        ...
        <div class="flex justify-end gap-3 mt-6">
          <button @click="editingUser = null" class="px-4 py-2 text-slate-400 hover:text-white text-sm transition-colors">Annuler</button>
