  ...
  <div class="p-6 max-w-6xl mx-auto space-y-6 animate-fade-in">
    ...
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-[var(--text-subtle)] text-sm mt-1">
      ...
      <Link href="/publications/externes"
        class="flex items-center gap-2 px-4 py-2 border border-[var(--border)] text-[var(--text-muted)] hover:text-[var(--text)] hover:bg-white/5 rounded-xl text-sm font-semibold transition-all">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
      <div class="bg-[var(--surface)] border border-[var(--border)] rounded-xl p-4 text-center">
        <p class="text-3xl font-extrabold text-[var(--text)]">{{ stats?.total?.toLocaleString('fr-FR') ?? '0' }}</p>
        <p class="text-xs font-semibold text-[var(--text-subtle)] uppercase tracking-wide mt-1">Articles indexés</p>
      </div>
      <div class="bg-[var(--surface)] border border-[var(--border)] rounded-xl p-4 text-center">
        <p class="text-3xl font-extrabold text-emerald-400">{{ stats?.disponible?.toLocaleString('fr-FR') ?? '0' }}</p>
        <p class="text-xs font-semibold text-[var(--text-subtle)] uppercase tracking-wide mt-1">Disponibles</p>
      </div>
      <div class="bg-[var(--surface)] border border-[var(--border)] rounded-xl p-4 text-center">
        <p class="text-3xl font-extrabold text-blue-400">{{ stats?.par_source?.length ?? 0 }}</p>
        <p class="text-xs font-semibold text-[var(--text-subtle)] uppercase tracking-wide mt-1">Sources actives</p>
      </div>
      <div class="bg-[var(--surface)] border border-[var(--border)] rounded-xl p-4 text-center">
        <p class="text-sm font-bold text-amber-400">{{ formatDate(stats?.dernier_import) }}</p>
        <p class="text-xs font-semibold text-[var(--text-subtle)] uppercase tracking-wide mt-1">Dernier import</p>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <!-- ─── Import manuel ─── -->
      <div class="bg-[var(--surface)] border border-[var(--border)] rounded-xl p-6 space-y-4">
        <h2 class="font-bold text-[var(--text)] flex items-center gap-2">
        ...
        <div>
          <label class="block text-xs font-medium text-[var(--text-subtle)] mb-1.5">Requête de recherche</label>
        ...
        <div class="grid grid-cols-2 gap-3">
          <div>
            <label class="block text-xs font-medium text-[var(--text-subtle)] mb-1.5">Source</label>
            <select v-model="form.source"
              class="w-full bg-[var(--surface-alt)]/60 border border-[var(--border)] rounded-lg px-3 py-2.5 text-sm text-[var(--text)] focus:outline-none focus:ring-2 focus:ring-blue-500/50">
          ...
          <div>
            <label class="block text-xs font-medium text-[var(--text-subtle)] mb-1.5">Limite par source</label>
            <input v-model="form.limit" type="number" min="5" max="200"
              class="w-full bg-[var(--surface-alt)]/60 border border-[var(--border)] rounded-lg px-4 py-2.5 text-sm text-[var(--text)] focus:outline-none focus:ring-2 focus:ring-blue-500/50" />
          </div>
        </div>
        ...
        <button @click="runImport" :disabled="running || form.processing"
          class="w-full flex items-center justify-center gap-2 px-5 py-3 bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-[var(--text)] rounded-xl text-sm font-bold transition-all hover:-translate-y-0.5">
      ...
      <div class="bg-[var(--surface)] border border-[var(--border)] rounded-xl p-6 space-y-4">
        ...
        <div v-if="stats?.par_source?.length > 0" class="space-y-3">
          <div v-for="src in stats.par_source" :key="src.source"
            class="flex items-center gap-4 p-4 bg-[var(--surface-alt)]/40 rounded-xl border border-[var(--border)]">
            <component :is="sourceIcons[src.source] ?? BookOpenIcon"
              class="w-8 h-8 shrink-0" :class="sourceColors[src.source] ?? 'text-[var(--text-subtle)]'" />
            <div class="flex-1">
              <p class="font-bold text-[var(--text)] text-sm">{{ sourceLabels[src.source] ?? src.source }}</p>
              <p class="text-[var(--text-subtle)] text-xs">Dernier fetch : {{ formatDate(src.last_fetch) }}</p>
            </div>
            <div class="text-right">
              <p class="text-xl font-extrabold text-[var(--text)]">{{ src.total?.toLocaleString('fr-FR') }}</p>
              <p class="text-[10px] text-[var(--text-subtle)] uppercase">articles</p>
            </div>
          </div>
        </div>
    ...
    <!-- ─── Logs d'import ─── -->
    <div class="bg-[var(--surface)] border border-[var(--border)] rounded-xl p-6">
      <h2 class="font-bold text-[var(--text)] flex items-center gap-2 mb-4">
        <ClockIcon class="w-5 h-5 text-amber-400" />
        Logs d'import (50 dernières lignes)
      </h2>
      <pre class="bg-black/60 rounded-xl border border-[var(--border)] p-4 text-xs text-green-400 font-mono overflow-auto max-h-64 leading-relaxed whitespace-pre-wrap">{{ importLog }}</pre>
    </div>

    <!-- ─── Derniers articles importés ─── -->
    <div v-if="stats?.derniers?.length > 0" class="bg-[var(--surface)] border border-[var(--border)] rounded-xl p-6">
      <h2 class="font-bold text-[var(--text)] flex items-center gap-2 mb-4">
      ...
      <div class="space-y-2">
        <div v-for="article in stats.derniers" :key="article.id"
          class="flex items-center gap-3 p-3 bg-[var(--surface-alt)]/30 rounded-lg border border-[var(--border)]">
          <span class="text-[10px] font-bold px-2 py-1 rounded-full border"
            :class="sourceColors[article.source] ?? 'text-[var(--text-subtle)]'">
            {{ sourceLabels[article.source] ?? article.source }}
          </span>
          <p class="text-sm text-[var(--text-muted)] flex-1 truncate">{{ article.titre }}</p>
          <span v-if="article.annee" class="text-xs text-[var(--text-subtle)] shrink-0">{{ article.annee }}</span>
