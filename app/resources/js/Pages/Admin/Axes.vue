  ...
  <div class="p-6 space-y-6 animate-fade-in">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
      <div v-for="axe in axes" :key="axe.id"
        class="bg-[var(--surface)] border border-[var(--border)] rounded-xl p-6 hover:border-purple-500/30 transition-all flex flex-col justify-between">
        <div>
          <div class="flex items-start justify-between mb-3">
            ...
            <div class="flex gap-3 text-xs text-[var(--text-subtle)]">
              <span>📄 {{ axe.publications_count }} pub.</span>
              <span>👥 {{ axe.membres_count }} membres</span>
            </div>
          </div>
          <h3 class="font-bold text-[var(--text)] text-lg">{{ axe.nom_fr }}</h3>
          <p class="text-sm text-[var(--text-subtle)] italic mt-0.5 mb-2">{{ axe.nom_en }}</p>
          <span class="inline-block px-2 py-0.5 bg-[var(--surface-alt)] border border-[var(--border)] rounded text-xs text-[var(--text-subtle)] font-mono">{{ axe.code }}</span>
          <p v-if="axe.description_fr" class="text-sm text-[var(--text-subtle)] mt-3 line-clamp-3">{{ axe.description_fr }}</p>
        </div>

        <div class="mt-4 pt-3 border-t border-[var(--border)] flex items-center justify-between text-xs text-slate-450">
          <div>
            <span v-if="axe.responsable" class="text-[var(--text-subtle)]">
              Responsable : <strong class="text-slate-200">{{ axe.responsable.prenom }} {{ axe.responsable.nom }}</strong>
            </span>
            <span v-else class="text-[var(--text-subtle)] italic">Aucun responsable</span>
          </div>
          <button v-if="props.userRole === 'super_admin'" @click="openEdit(axe)"
            class="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-[var(--surface-alt)] hover:bg-slate-700 text-[var(--text-muted)] font-medium transition-colors">
    ...
    <div v-if="showCreateModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" @click.self="showCreateModal = false">
      <div class="bg-[var(--surface)] border border-[var(--border)] rounded-2xl p-6 w-full max-w-lg shadow-glass animate-slide-up max-h-[95vh] overflow-y-auto">
        <div class="flex items-center justify-between mb-4 pb-3 border-b border-[var(--border)]">
          <h3 class="text-lg font-semibold text-[var(--text)]">Créer un axe thématique</h3>
          <button @click="showCreateModal = false" class="text-[var(--text-subtle)] hover:text-[var(--text)] transition-colors">
        ...
        <form @submit.prevent="saveCreate" class="space-y-4">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label class="block text-xs font-medium text-[var(--text-subtle)] mb-1.5">Code de l'axe (unique)</label>
              <input v-model="createForm.code" required placeholder="ex: epidemio"
                class="w-full bg-slate-850 border border-[var(--border)] rounded-lg px-4 py-2 text-sm text-[var(--text)] focus:outline-none focus:ring-2 focus:ring-brand-500/50 transition-all" />
              <div v-if="createForm.errors.code" class="text-red-400 text-xs mt-1">{{ createForm.errors.code }}</div>
            </div>
            <div>
              <label class="block text-xs font-medium text-[var(--text-subtle)] mb-1.5">Couleur</label>
              <div class="flex gap-2">
                <input type="color" v-model="createForm.couleur_hex"
                  class="w-10 h-9 bg-slate-850 border border-[var(--border)] rounded-lg p-1 cursor-pointer" />
                <input v-model="createForm.couleur_hex" placeholder="#3B82F6"
                  class="flex-1 bg-slate-850 border border-[var(--border)] rounded-lg px-4 py-2 text-sm text-[var(--text)] focus:outline-none focus:ring-2 focus:ring-brand-500/50 transition-all" />
              </div>
          ...
          <div>
            <label class="block text-xs font-medium text-[var(--text-subtle)] mb-1.5">Nom (Français)</label>
          ...
          <div>
            <label class="block text-xs font-medium text-[var(--text-subtle)] mb-1.5">Nom (Anglais)</label>
          ...
          <div>
            <label class="block text-xs font-medium text-[var(--text-subtle)] mb-1.5">Description (Français)</label>
            <textarea v-model="createForm.description_fr" rows="3" placeholder="Description de l'axe en français..."
              class="w-full bg-slate-850 border border-[var(--border)] rounded-lg px-4 py-2 text-sm text-[var(--text)] focus:outline-none focus:ring-2 focus:ring-brand-500/50 transition-all"></textarea>
            <div v-if="createForm.errors.description_fr" class="text-red-400 text-xs mt-1">{{ createForm.errors.description_fr }}</div>
          </div>

          <div>
            <label class="block text-xs font-medium text-[var(--text-subtle)] mb-1.5">Description (Anglais)</label>
            <textarea v-model="createForm.description_en" rows="3" placeholder="Description de l'axe en anglais..."
              class="w-full bg-slate-855 border border-[var(--border)] rounded-lg px-4 py-2 text-sm text-[var(--text)] focus:outline-none focus:ring-2 focus:ring-brand-500/50 transition-all"></textarea>
            <div v-if="createForm.errors.description_en" class="text-red-400 text-xs mt-1">{{ createForm.errors.description_en }}</div>
          </div>

          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label class="block text-xs font-medium text-[var(--text-subtle)] mb-1.5">Responsable d'axe (Chercheur)</label>
              <select v-model="createForm.responsable_id"
                class="w-full bg-slate-850 border border-[var(--border)] rounded-lg px-4 py-2 text-sm text-slate-200 focus:outline-none focus:ring-2 focus:ring-brand-500/50 transition-all">
            ...
            <div>
              <label class="block text-xs font-medium text-[var(--text-subtle)] mb-1.5">Ordre d'affichage</label>
          ...
          <div class="flex items-center gap-2 pt-2">
            <input type="checkbox" id="actif_create" v-model="createForm.actif"
              class="w-4 h-4 rounded border-[var(--border)] bg-slate-850 text-brand-600 focus:ring-brand-500/55 focus:ring-offset-slate-900 focus:outline-none" />
            <label for="actif_create" class="text-sm text-[var(--text-muted)] font-medium cursor-pointer">Axe actif</label>
          </div>

          <div class="flex justify-end gap-3 mt-6 pt-4 border-t border-[var(--border)]">
            <button type="button" @click="showCreateModal = false" class="px-4 py-2 text-[var(--text-subtle)] hover:text-[var(--text)] text-sm transition-colors">Annuler</button>
    ...
    <div v-if="editingAxe" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" @click.self="editingAxe = null">
      <div class="bg-[var(--surface)] border border-[var(--border)] rounded-2xl p-6 w-full max-w-lg shadow-glass animate-slide-up max-h-[95vh] overflow-y-auto">
        <div class="flex items-center justify-between mb-4 pb-3 border-b border-[var(--border)]">
          <h3 class="text-lg font-semibold text-[var(--text)]">Modifier l'axe thématique</h3>
          <button @click="editingAxe = null" class="text-[var(--text-subtle)] hover:text-[var(--text)] transition-colors">
            <XMarkIcon class="w-5 h-5" />
          </button>
        </div>
        ...
        <form @submit.prevent="saveEdit" class="space-y-4">
          ...
          <div>
            ...
            <input v-model="editForm.nom_fr" required placeholder="ex: Épidémiologie et Modélisation"
              class="w-full bg-slate-855 border border-[var(--border)] rounded-lg px-4 py-2 text-sm text-[var(--text)] focus:outline-none focus:ring-2 focus:ring-brand-500/50 transition-all" />
          ...
          <div class="flex items-center gap-2 pt-2">
            <input type="checkbox" id="actif_edit" v-model="editForm.actif"
              class="w-4 h-4 rounded border-[var(--border)] bg-slate-850 text-brand-600 focus:ring-brand-500/55 focus:ring-offset-slate-900 focus:outline-none" />
            <label for="actif_edit" class="text-sm text-[var(--text-muted)] font-medium cursor-pointer">Axe actif</label>
          </div>

          <div class="flex justify-end gap-3 mt-6 pt-4 border-t border-[var(--border)]">
            <button type="button" @click="editingAxe = null" class="px-4 py-2 text-[var(--text-subtle)] hover:text-[var(--text)] text-sm transition-colors">Annuler</button>
