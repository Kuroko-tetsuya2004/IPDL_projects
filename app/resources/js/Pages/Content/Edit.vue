...
const form = useForm({
  titre_fr:       props.publication.titre_fr ?? '',
  titre_en:       props.publication.titre_en ?? '',
  resume_fr:      props.publication.resume_fr ?? '',
  resume_en:      props.publication.resume_en ?? '',
  type:           props.publication.type ?? 'article',
  axe_id:         props.publication.axe_id ?? '',
  mots_cles:      props.publication.mots_cles ?? '',
  date_publication: props.publication.date_publication ? props.publication.date_publication.substring(0, 10) : '',
  doi:            props.publication.doi ?? '',
  url_externe:    props.publication.url_externe ?? '',
  visibilite:     props.publication.visibilite ?? 'public',
  langue_contenu: props.publication.langue_principale ?? 'fr',
  commentaire_auteur: '',
  fichier:        null,
})
...
const submit = () => {
  form.transform((data) => ({
    ...data,
    _method: 'PUT',
  })).post(`/publications/${props.publication.id}`, {
    onSuccess: () => {},
  })
}
  ...
  <div class="p-6 max-w-3xl mx-auto space-y-6 animate-fade-in">
    <div>
      ...
      <p v-if="isDoctorant" class="text-amber-400 text-sm mt-2 flex items-center gap-2">
        ⚠️ La modification remettra cette publication dans le workflow de validation.
      </p>
      <p v-else class="text-slate-400 text-sm mt-2">
        Vos modifications seront publiées directement (chercheur).
      </p>
    </div>
    ...
    <form @submit.prevent="submit" class="space-y-6">
      ...
      <div class="bg-slate-900/60 border border-white/8 rounded-xl p-6 space-y-4">
        ...
        <div>
          <label class="block text-xs font-medium text-slate-400 mb-1.5">Remplacer le document joint (optionnel)</label>
      ...
      <div class="bg-slate-900/60 border border-white/8 rounded-xl p-6 space-y-4">
        ...
        <div v-if="isDoctorant" class="pt-2">
          <label class="block text-xs font-medium text-slate-400 mb-1.5">Commentaire pour le validateur (optionnel)</label>
          <textarea v-model="form.commentaire_auteur" rows="3" placeholder="Décrivez les modifications apportées..."
            class="w-full bg-slate-800/60 border border-white/10 rounded-lg px-4 py-2.5 text-sm text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-brand-500/50 transition-all resize-none"></textarea>
        </div>
      </div>
