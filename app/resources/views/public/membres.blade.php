...
<div class="container" style="padding-top: 2rem;">
    ...
    <div style="margin-bottom: 4rem;">
        <h2 style="font-size: 1.6rem; font-weight: 700; margin-bottom: 2rem; color: var(--color-primary); display: flex; align-items: center; gap: 0.5rem;">
                ...
                <div class="card" style="display: flex; flex-direction: column; justify-content: space-between; height: 100%;">
                    <div>
                        <div style="display: flex; align-items: center; gap: 1rem; margin-bottom: 1.25rem;">
                            @if($c->photo_url)
                                <img src="{{ $c->photo_url }}" alt="Photo de {{ $c->prenom }} {{ $c->nom }}" style="width: 60px; height: 60px; border-radius: 50%; object-fit: cover; border: 2px solid var(--color-primary-light);">
                            @else
                                <div style="width: 60px; height: 60px; border-radius: 50%; background: rgba(37, 99, 235, 0.08); color: var(--color-primary-light); display: flex; align-items: center; justify-content: center; font-size: 1.25rem; font-weight: 700; border: 2px solid rgba(37, 99, 235, 0.15);">
                        ...
                        @if($c->biographie_fr)
                            <p style="font-size: 0.85rem; color: var(--color-text-muted); line-height: 1.6; margin-bottom: 1rem;">
                                {{ Str::limit($c->biographie_fr, 150) }}
                            </p>
                        @else
                            <p style="font-size: 0.85rem; color: var(--color-text-muted); font-style: italic; margin-bottom: 1rem;">
                    ...
                    {{-- Social links & ORCID --}}
                    <div style="border-top: 1px solid var(--color-border); padding-top: 0.75rem; margin-top: 0.5rem; display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 0.5rem;">
                        <div>
                            @if($c->orcid_id)
            ...
            <div class="grid-3">
                ...
                <div class="card" style="display: flex; flex-direction: column; justify-content: space-between; height: 100%;">
                    <div>
                        <div style="display: flex; align-items: center; gap: 1rem; margin-bottom: 1.25rem;">
                            @if($d->photo_url)
                                <img src="{{ $d->photo_url }}" alt="Photo de {{ $d->prenom }} {{ $d->nom }}" style="width: 50px; height: 50px; border-radius: 50%; object-fit: cover; border: 2px solid var(--color-primary-light);">
                            @else
                                <div style="width: 50px; height: 50px; border-radius: 50%; background: rgba(37, 99, 235, 0.05); color: var(--color-primary-light); display: flex; align-items: center; justify-content: center; font-size: 1.1rem; font-weight: 700; border: 2px solid rgba(37, 99, 235, 0.1);">
                                    {{ substr($d->prenom, 0, 1) }}{{ substr($d->nom, 0, 1) }}
                                </div>
                            @endif
                            <div>
                                <h3 style="font-size: 1rem; font-weight: 700; color: var(--color-text);">
                                    {{ $d->prenom }} {{ $d->nom }}
                                </h3>
